  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  if (traj_point_active_ptr_ && (*traj_point_active_ptr_)->has_trajectory_msg())
  {
    ...
    if(!joint_limit_tolerated){
      joint_limit_tolerated |= check_state_limit_violations(state_current_);
    }
    ...
    if(!joint_limit_tolerated){
      ...
      if (valid_point)
      {
        ...
        for (size_t index = 0; index < dof_; ++index)
        {
              ...
              state_error_, index, default_tolerances_.state_tolerance[index], true))
          {
            ...
            RCLCPP_WARN(get_node()->get_logger(),"tolerance violated for joint: %d", index);
          }
      ...
      // RCLCPP_INFO(get_node()->get_logger(), "Joint limits tolerated sending stop command");
      control_state_.clear();
      for (size_t i = 0; i < dof_; ++i){
        control_state_.push_back(2.0); // quick stop
      }
...
bool JointTrajectoryController::check_state_limit_violations(JointTrajectoryPoint & state){

  bool limit_tolerated_status = false;

  for (size_t index = 0; index < dof_; ++index){

    // RCLCPP_INFO(get_node()->get_logger(), "Joint %d pos value %f[lower %f upper %f]", index, state.positions[index],lower_joint_limits_[index],upper_joint_limits_[index]);
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  for (size_t i = 0; i < dof_; ++i)
  {
    ...
    auto lower_joint_limit = params_.joint_limits.joints_map.at(joint).lower_limit;
    auto upper_joint_limit = params_.joint_limits.joints_map.at(joint).upper_limit;

    lower_joint_limits_.push_back(lower_joint_limit);
    upper_joint_limits_.push_back(upper_joint_limit);

    RCLCPP_INFO(logger, "Joint %d limits: [lower %f, upper %f]", i ,lower_joint_limit, upper_joint_limit);
  }

...
controller_interface::CallbackReturn DynamicConveyorController::on_init() {
    ...
    gantry_lift_request_available_ = false;
...
controller_interface::InterfaceConfiguration DynamicConveyorController::state_interface_configuration() const {
    ...
    conf_names.push_back(params_.left_lift_actuator_name + "/" + "position");
    conf_names.push_back(params_.right_lift_actuator_name + "/" + "position");
    conf_names.push_back(params_.left_lift_actuator_name + "/" + "effort");
    conf_names.push_back(params_.right_lift_actuator_name + "/" + "effort");
...
controller_interface::return_type DynamicConveyorController::update(const rclcpp::Time & time, const rclcpp::Duration &) {
    ...
    if(gantry_lift_request_available_) {
        RCLCPP_INFO(get_node()->get_logger(), "gantry lift request available");

        left_gantry_target_distance_ = gantry_travel_distance_ - left_gantry_initial_encoder_offset_;
        right_gantry_target_distance_ = gantry_travel_distance_ - right_gantry_initial_encoder_offset_ - left_minus_right_travel_offset_;
        
        if(left_gantry_target_distance_ > left_gantry_raw_distance_) {
            left_final_moveback_distance_ = params_.final_moveback_distance;
        } else {
            left_final_moveback_distance_ = -1 * params_.final_moveback_distance;
        }
        if(right_gantry_target_distance_ > right_gantry_raw_distance_) {
            right_final_moveback_distance_ = params_.final_moveback_distance;
        } else {
            right_final_moveback_distance_ = -1 * params_.final_moveback_distance;
        }


        RCLCPP_INFO(get_node()->get_logger(), "gantry lift target distances: L:%f, R:%f", left_gantry_target_distance_, right_gantry_target_distance_);
...
controller_interface::CallbackReturn DynamicConveyorController::on_configure(
    ...
    last_commanded_belt_velocity_ = params_.initial_belt_speed;

    left_actuator_current_storage_.clear();
    left_actuator_current_storage_.reserve(params_.current_storage_window);
    right_actuator_current_storage_.clear();
    right_actuator_current_storage_.reserve(params_.current_storage_window);
...
controller_interface::CallbackReturn DynamicConveyorController::on_activate(
    ...
    std::vector<std::string> required_command_interfaces_name {
        ...
        params_.left_lift_actuator_name + "/" + "effort",
        params_.right_lift_actuator_name + "/" + "effort",
...
bool DynamicConveyorController::current_sanity_check() {
